...
resdir=paste0(proj_dir,"res/twogrp/")
setwd(resdir)
# all day to avg day
...
feat_general_list=c("age","sex","ethnicity","alcohol","exercise","edu_yr","bmi")#features from meta data, demographic
...
# 
metatest_tab2=read.table(paste0(proj_dir,"data/meta_lab_1_old.csv"),header=TRUE,sep=",")
metatest_tab2=metatest_tab2[,c("subject.id","alcohol","edu_yr","bmi_bl")]
metatest_tab2[,"subject.id"]<-metatest_tab2[,"subject.id"]%>%str_remove(string=.,pattern="43883-")%>%as.numeric()
# new meta table
metatest_tab=read.table(paste0(proj_dir,"data/combined_meta_data_all_final.csv"),header=TRUE,sep=",")
# metatest_tab=metatest_tab[metatest_tab[,"subject.id"]%in%subjid,]
metatest_tab=metatest_tab[metatest_tab[,"cohort"]=="original",]
metatest_tab=merge(metatest_tab,metatest_tab2,by="subject.id")
